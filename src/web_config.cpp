...
static void handleUpdateGet() {
    String html = F(
        ...
        "<title>WireClaw Firmware Update</title>"
        "<style>"
        "*{box-sizing:border-box;margin:0;padding:0}"
        ":root{--bg:#08090e;--bg2:#0d1019;--bg3:#141822;"
        "--accent:#00d4aa;--accent-dim:rgba(0,212,170,0.15);--accent-glow:rgba(0,212,170,0.25);"
        "--text:#e8eaf0;--text2:#8b92a8;--text3:#4a5068;"
        "--border:rgba(255,255,255,0.06);--border-a:rgba(0,212,170,0.25);"
        "--red:#ff4757;"
        ...
        "nav .navlink{margin-left:auto;display:inline-flex;align-items:center;background:var(--accent-dim);"
        "color:var(--accent);font-size:0.8rem;font-weight:600;text-decoration:none;padding:0.4rem 0.9rem;"
        "border-radius:8px;border:1px solid var(--border-a);transition:all 0.15s}"
        "nav .navlink:hover{background:var(--accent);color:var(--bg)}"
        ".card{background:var(--bg3);border:1px solid var(--border);border-radius:12px;padding:1.5rem;margin-bottom:1rem}"
        "label{display:block;font-size:0.8rem;color:var(--accent);font-weight:600;margin:1rem 0 0.25rem;"
        "font-family:var(--mono);text-transform:uppercase;letter-spacing:0.04em}"
        "label:first-child{margin-top:0}"
        "input[type=file]{width:100%;padding:0.6rem 0.75rem;background:var(--bg2);border:1px solid var(--border);"
        "border-radius:8px;color:var(--text);font-family:var(--mono);font-size:0.85rem;transition:border-color 0.15s}"
        "input[type=file]:focus{outline:none;border-color:var(--border-a)}"
        ".hint{font-size:0.75rem;color:var(--text3);margin-top:0.4rem}"
        ".btn{display:inline-flex;align-items:center;gap:0.5rem;padding:0.6rem 1.25rem;border:none;"
        "border-radius:8px;font-weight:600;font-size:0.9rem;cursor:pointer;transition:all 0.2s;font-family:var(--font)}"
        ".btn-primary{background:var(--accent);color:var(--bg)}"
        ".btn-primary:hover{box-shadow:0 0 20px var(--accent-glow)}"
        ".btn-primary:disabled{opacity:0.5;cursor:not-allowed;box-shadow:none}"
        ".actions{display:flex;gap:0.75rem;margin-top:1.25rem;flex-wrap:wrap}"
        "#s{margin-top:1rem;font-family:var(--mono);font-size:0.85rem;color:var(--text2);white-space:pre-wrap}"
        "</style></head><body>"
        "<div class=\"wrap\">"
        "<header><h1>WireClaw</h1><span class=\"ver\">" WIRECLAW_VERSION "</span></header>"
        "<nav><a class=\"navlink\" href=\"/\">Back to Config</a></nav>"
        "<div class=\"card\">"
        "<h2 style=\"font-size:1.1rem;font-weight:700;margin-bottom:0.5rem\">Firmware Update</h2>"
        "<p class=\"hint\" style=\"margin-bottom:1rem\">Select a <b>firmware.bin</b> (app image) and upload it. "
        "The device writes it to the inactive OTA partition and reboots automatically. Your config is preserved.</p>"
